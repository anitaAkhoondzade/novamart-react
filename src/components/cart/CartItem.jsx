      ...
      <div className="flex-1 flex justify-between items-center gap-4">
        <div>
          ...
          <p className="mt-2 text-2xl font-bold text-purple-600">
           Subtotal: £{(cartItem.product.price * cartItem.quantity).toLocaleString()}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => decreaseQuantity(cartItem.product.id)}>-</button>
          <p className="text-lg font-semibold">{cartItem.quantity}</p>
          <button onClick={() => increaseQuantity(cartItem.product.id)}>+</button>
